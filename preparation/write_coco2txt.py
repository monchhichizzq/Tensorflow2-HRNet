...
def parse_arguments():
    parser = argparse.ArgumentParser(description='To convert coco annotations to txt files')
    parser.add_argument('-coco_root', default='/Users/babalia/Desktop/Applications/HRNet/coco_dataset', 
                        help="COCO root directory", action="store_true")
    parser.add_argument('-txt_save', default='data_txt/coco', 
                        help="COCO annotations txt files", action="store_true")
...
class COCO_keypoints(object):
    def __init__(self, coco_root_dir, txt_dir):
        self.txt_dir = txt_dir
        os.makedirs(self.txt_dir, exist_ok=True)
        self.annotation = coco_root_dir + "/annotations/"
        self.images_dir = coco_root_dir + "/train2017/"
    ...
    @staticmethod # 放在函数前（该函数不传入self或者cls），所以不能访问类属性和实例属性
    ...
    def write_information_to_txt(self, dataset):
        if dataset == "train":
            ...
            txt_file = os.path.join(self.txt_dir, 'coco_train.txt')
        elif dataset == "valid":
            data_dict = self.valid_dict
            txt_file = os.path.join(self.txt_dir, 'coco_valid.txt')
        else:
        ...
        with open(file=txt_file, mode="a+") as f:
            for i in range(len(image_ids_from_keypoints)):
                ...
                one_human_instance_info += self.__get_the_path_of_picture(image_files[image_index]) + " " # 0 image_path
                one_human_instance_info += str(image_heights[image_index]) + " " # 1 height
                one_human_instance_info += str(image_widths[image_index]) + " "  # 2 width
                one_human_instance_info += self.__list_to_str(bboxes[i]) + " "   # 3-6 x, y, w, h
                one_human_instance_info += self.__list_to_str(keypoints_list[i]) # 7- pose
...
if __name__ == '__main__':
    ...
    txt_dir = args.txt_save

    coco = COCO_keypoints(coco_root_dir, txt_dir)
...
'''
coco 数据集 人体关键点
生成txt文件, 每张图片中有多组人体关键点, 每组关键点占据一行

是否使用@staticmethod?
- 您是否希望在没有实例的情况下从外部访问此函数？大多数时候?
使用方法:

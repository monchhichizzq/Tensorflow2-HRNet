...
def read_image(image_dir):
    ...
    image = tf.io.decode_image(contents=image_content, channels=3, dtype=tf.dtypes.float32)
    return image
...
class ResizeTransform(object):
    def __init__(self, image, keypoints, bbox, resize_h, resize_w, num_of_joints, visual):
        self.image = image
        self.keypoints = keypoints
        self.bbox = bbox
        self.resize_h = resize_h
    ...
    def image_transform(self):
        human_instance = tf.image.crop_to_bounding_box(image=self.image,
                                                       ...
                                                       target_width=self.bbox[2])
        # if self.visual:
        #     print('human_instance: {} - {}'.format(np.min(human_instance), np.max(human_instance)))
        #     human_instance = np.array(human_instance*255, dtype=np.uint8)
        #     cv2.imshow('human instance', human_instance)
        #     cv2.waitKey(1000)
        ...
        if self.visual:
            ...
            cv2.imshow('human instance', resized_image)
            cv2.waitKey(1000)
